...
# Read input folder (either from arguments or given in script)
ARGS = commandArgs(trailingOnly = T)
if (length(ARGS) == 0 ){
  example_folder = '/gpfs42/projects/lab_anavarro/disease_pleiotropies/anthropometric/anna/pleiotropy/2-PLEIO/example'
  out_folder = '/gpfs42/projects/lab_anavarro/disease_pleiotropies/anthropometric/anna/pleiotropy/2-PLEIO/example/plot'
} else {example_folder = ARGS[1]}

# Library
library(pleiotropyPlot)

# Save input paths, each to one variable
...
input_parser = function(pleioin, pleiores, sumstats, snps, h2, rg, traits, ref){
  ...
  dat$outf = outf
    ...
    # Get trait name from the file name (I think)
...
main = function(dat){
  ...
  for ( i in 1:length(dat$snps)){
    ...
    png(file.path(outf,paste(snp,'.png',sep='')), width = 7, height = 7, units = 'in', res = 300, type='cairo')
    par(mar =c(0,0,0,0))
